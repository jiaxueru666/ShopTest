...
public class MainActivity extends AppCompatActivity {
    ...
    private List<ShopCartBean.OrderDataBean.CartlistBean> mAllOrderList = new ArrayList<>();
    private ArrayList<ShopCartBean.OrderDataBean.CartlistBean> mGoPayList = new ArrayList<>();
            ...
            public void onResfresh( boolean isSelect) {
                ...
                for(int i = 0;i < mAllOrderList.size(); i++)
                    if(mAllOrderList.get(i).getIsSelect()   ) {
                        mTotalPrice += Float.parseFloat(String.valueOf(mAllOrderList.get(i).getPrice())) * mAllOrderList.get(i).getCount();
                        mTotalNum += 1;
    ...
    private void initData() {
        try{
            ...
            InputStream inputStream =  this.getAssets().open("shop.json");
            String data =  StringUtils.convertStreamToString(inputStream);
            Gson gson = new Gson();
            ShopCartBean shopBean =  gson.fromJson(data, ShopCartBean.class);
...
//    private void initData(){
//        for(int i = 0;i < 2;i ++){
//            ShopCartBean.OrderDataBean.CartlistBean sb = new ShopCartBean.OrderDataBean.CartlistBean();
//            sb.setShopId(1);
//            sb.setPrice("1.0");
...
//    }
    ...
    public static void isSelectFirst(List<ShopCartBean.OrderDataBean.CartlistBean> list){
        if(list.size() > 0) {
